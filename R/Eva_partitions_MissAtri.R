#' @title
#' ELVAR algorithm can predict missing nodes attribute using the other nodes attribute.
#'
#' @aliases Eva_partitions_MissAtri
#'
#' @description
#' The code is an extended form of Eva_partitions() function.
#' It infers missing attribute of nodes from a partially node-attributed graph.
#' Missing attribute of nodes are assigned as "MA".
#'
## Use Eva_partitions_MissAtri() function to run the code...
#'
## Input parameters......
#' @param G
...
#'
#'
Eva_partitions_MissAtri <- function(G, resolution= 1, threshold= 0.0000001, alpha=0.5, order=NULL, Vattr.name){
  ...
  while(modified & nb_pass_done != -1){
    ...
    for(u in rand_nodes){
      ...
      for(kk in sample(seq(dim(weights2com)[1]),  dim(weights2com)[1], replace = FALSE)){
        ...
        #
        if(length(names(initital_labels[[1]]))==1 && names(initital_labels[[1]])== "MA"){
          total_incr <- gain
          #
          if(total_incr > best_mod){
            best_mod <- total_incr
...
one_levelSecond <- function(G, resolution, threshold, Grph_status, alpha, order){
  ...
  while(modified & nb_pass_done != -1){
    ...
    for(u in rand_nodes){
      ...
      Stot[best_com] <- Stot[best_com] + degrees
      #
      Prev_inner_partition <- inner_partition
      Prev_Grph_status <- Grph_status
      #
      #
      if(best_com != node2com[u]){
        #
    ...
    if(score <= threshold){
      # partition <- partition[which(lengths(partition)!=0)]
      inner_partition <- Prev_inner_partition[which(lengths(Prev_inner_partition)!=0)]
      clusterInfo <- Prev_Grph_status$commu[which(lengths(Prev_Grph_status$commu)!=0)]
      # return(list(partition=partition, inner_partition= inner_partition, modified= modified, Modu = new_mod))
